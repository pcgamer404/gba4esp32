...
/* Bytes of `rom` that are safe to read. The ESP port sets this to the real
 * flashed size before emuInit (its sparse mapping faults past the cart);
 * other ports leave it 0 and the scan bounds itself conservatively. */
uint32_t espgba_rom_size = 0;

/* Find `pat` in the ROM (plain bounded memmem: the ROM is full of zero
 * bytes, so strstr cannot be used). */
