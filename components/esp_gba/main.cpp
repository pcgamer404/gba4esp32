...
void systemOnWriteDataToSoundBuffer(int16_t *finalWave, int length) {
  ...
  /* Keep audioSamples: the frame limiter uses it as an emulated-time clock. */
  if (length > 0) {
    audioSamples += (uint32_t)(length / 2);
  }

#if !MY_RETROGO_NO_AUDIO
  if (length <= 0) return;
...
void emuInit() {
  // Initialize the per-game settings required by the ESP32-S3 port.
...
extern "C" void app_main() {
  ...
  #if !MY_RETROGO_NO_AUDIO
  /* Audio hardware disabled on MY_RETROGO.
   * Its GPIOs overlap the real control buttons. */
  audioI2cInit();
  audioI2cScan();
  audioInit(47872);
  audioDumpRegs();
  ...
  if (sdOk) {
    ...
    if (nRoms > 0) {
      ...
      menuMessage("no ROMs in /sd/roms", "using flashed rom");
      delayMS(2500);
    } else {
      menuMessage("no ROMs in /sd/roms", "add gb/gbc/gba, reboot");
      delayMS(15000);
      esp_restart();
    }
  } else if (haveFlashed) {
  ...
  while (1) {
	...
	/* Hold SELECT + START for 1.5 seconds to leave the game
 * and return to the ROM picker. */
{
  static int64_t exitHoldStart = 0;
  ...
  if (exitCombo) {
    if (exitHoldStart == 0) {
      exitHoldStart = esp_timer_get_time();
    } else if (esp_timer_get_time() - exitHoldStart >= 1500000) {
      saveFlush();
...
#if !defined(HEADLESS) && defined(PANEL_ILI9341) && LCD_HAS_READBACK
