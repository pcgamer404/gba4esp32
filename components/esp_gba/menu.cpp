 ...
 *
 * Navigation uses the handheld's physical buttons. The same input mapping is
 * used by the emulator and the menu.
 */
...
/* ------------------------------------------------------------------ */
/* In-game menu. SELECT+START opens this screen.                       */
/* The menu is intentionally generic so more per-game options can be    */
/* added later without changing the GB core.                            */
...
#define GAME_SPEED_COUNT 4
static const uint8_t gameSpeedX10[GAME_SPEED_COUNT] = {10, 15, 20, 25};

static uint32_t gameMenuHash(const char *s) {
...
static uint8_t gameSpeedLoad(const char *gameName) {
  nvs_handle_t h;
  uint8_t v = 0;
  char key[12];
  snprintf(key, sizeof(key), "s%08lx",
           (unsigned long)gameMenuHash(gameName));
  if (nvs_open("gbgame", NVS_READONLY, &h) == ESP_OK) {
    nvs_get_u8(h, key, &v);
    nvs_close(h);
  }
  if (v >= GAME_SPEED_COUNT) {
    v = 0;
  }
  return v;
}

static void gameSpeedStore(const char *gameName, uint8_t v) {
  nvs_handle_t h;
  char key[12];
  snprintf(key, sizeof(key), "s%08lx",
           (unsigned long)gameMenuHash(gameName));
  if (nvs_open("gbgame", NVS_READWRITE, &h) == ESP_OK) {
    nvs_set_u8(h, key, v);
    nvs_commit(h);
    nvs_close(h);
  }
...
static void gameStatePath(const char *gameName, int slot, char *out, size_t outLen) {
  char base[260];
  snprintf(base, sizeof(base), "%s", gameName ? gameName : "game");
  char *dot = strrchr(base, '.');
  if (dot) {
    *dot = 0;
  }
  snprintf(out, outLen, "/sd/saves/%s.state%d", base, slot);
}

static bool gameStateExists(const char *gameName, int slot) {
  char path[300];
  gameStatePath(gameName, slot, path, sizeof(path));
...
static const char *gameSpeedText(int speedIndex) {
  static const char *const text[GAME_SPEED_COUNT] = {
      "1.0x", "1.5x", "2.0x", "2.5x"};
  if (speedIndex < 0 || speedIndex >= GAME_SPEED_COUNT) {
    speedIndex = 0;
  }
  return text[speedIndex];
}

static int gGameMenuSel;
static int gGameMenuSpeed;
static const char *gGameMenuName;
...
static void drawGameMenu(void) {
  ...
  menuClear(0x0000);
  menuText(16, 6, "GAME SETTINGS", 0xFFFF);

  char title[33];
  snprintf(title, sizeof(title), "%.30s", gGameMenuName ? gGameMenuName : "game");
  menuText(16, 18, title, 0x7BEF);

  for (int i = 0; i < 9; i++) {
    int y = 40 + i * 20;
    if (i == gGameMenuSel) {
      menuBar(10, y - 2, MENU_W - 20, 18, 0x18E3);
    }
    menuText(20, y, rows[i], 0xFFFF);
    if (i == 0) {
      menuText(MENU_W - 52, y, gameSpeedText(gGameMenuSpeed), 0x07E0);
    } else if (i == 2) {
      menuText(MENU_W - 64, y,
               gameStateExists(gGameMenuName, 1) ? "READY" : "EMPTY",
               gameStateExists(gGameMenuName, 1) ? 0x07E0 : 0xF800);
    } else if (i == 4) {
      menuText(MENU_W - 64, y,
               gameStateExists(gGameMenuName, 2) ? "READY" : "EMPTY",
               gameStateExists(gGameMenuName, 2) ? 0x07E0 : 0xF800);
    }
  }

  menuText(12, MENU_H - 10, "A select / L-R speed / B resume", 0x39E7);
}

int menuGameSettings(const char *gameName, int *speedIndex) {
  gGameMenuName = gameName;
  gGameMenuSpeed = gameSpeedLoad(gameName);
  if (speedIndex) {
    *speedIndex = gGameMenuSpeed;
  }
  gGameMenuSel = 0;
  menuRender(drawGameMenu);

  /* SELECT+START is still physically held when this function is entered.
   * Wait for both buttons to be released so the opening combo never becomes
   * an accidental menu selection. */
  while (osReadKey() & ((1u << 2) | (1u << 3))) {
    vTaskDelay(pdMS_TO_TICKS(10));
  }

  uint32_t lastKeys = osReadKey();
  while (1) {
    osPollSerial();
    ...
    if (pressed & (1u << 6)) { /* up */
      gGameMenuSel = (gGameMenuSel + 8) % 9;
      menuRender(drawGameMenu);
    }
    if (pressed & (1u << 7)) { /* down */
      gGameMenuSel = (gGameMenuSel + 1) % 9;
      menuRender(drawGameMenu);
    }

    if (gGameMenuSel == 0 &&
        (pressed & ((1u << 5) | (1u << 4)))) { /* left/right */
      int dir = (pressed & (1u << 4)) ? 1 : -1;
      gGameMenuSpeed = (gGameMenuSpeed + GAME_SPEED_COUNT + dir) % GAME_SPEED_COUNT;
      gameSpeedStore(gameName, (uint8_t)gGameMenuSpeed);
      if (speedIndex) {
        *speedIndex = gGameMenuSpeed;
      }
      menuRender(drawGameMenu);
    ...
    if (pressed & (1u << 0)) { /* A */
      if (gGameMenuSel == 0) {
        gGameMenuSpeed = (gGameMenuSpeed + 1) % GAME_SPEED_COUNT;
        gameSpeedStore(gameName, (uint8_t)gGameMenuSpeed);
        if (speedIndex) {
          *speedIndex = gGameMenuSpeed;
        }
        menuRender(drawGameMenu);
      } else if (gGameMenuSel == 1) {
        return GAME_MENU_SAVE_STATE1;
      } else if (gGameMenuSel == 2) {
        return GAME_MENU_LOAD_STATE1;
      } else if (gGameMenuSel == 3) {
        return GAME_MENU_SAVE_STATE2;
      } else if (gGameMenuSel == 4) {
        return GAME_MENU_LOAD_STATE2;
      } else if (gGameMenuSel == 5) {
        return GAME_MENU_SAVE;
      } else if (gGameMenuSel == 6) {
        return GAME_MENU_RESET;
      } else if (gGameMenuSel == 7) {
        return GAME_MENU_EXIT;
      } else {
        return GAME_MENU_RESUME;
      }
    }

    if (pressed & (1u << 1)) { /* B = resume */
      return GAME_MENU_RESUME;
    }
...
#define NVS_KEY_ROM_SIZE "romsize"
...
uint32_t romGetFlashedSize(void) {
  ...
  uint32_t v = 0;
  nvs_get_u32(h, NVS_KEY_ROM_SIZE, &v);
  nvs_close(h);
  return v;
}
...
void romInvalidateFlashed(void) {
  ...
  nvs_erase_key(h, NVS_KEY_ROM_SIZE);
...
static void romSetFlashed(const char *name, const char *code) {
  ...
  nvs_set_u32(h, NVS_KEY_ROM_SIZE, gRomSize);
  nvs_commit(h);
  nvs_close(h);
}
...
bool romCopyFromSd(const char *name, uint32_t size, const char *code) {
  ...
  const char *romDir = SD_ROM_GBA_DIR;
  const char *dot = strrchr(name, '.');

  if (dot && strcasecmp(dot, ".gb") == 0) {
  romDir = SD_ROM_GB_DIR;
  } else if (dot && strcasecmp(dot, ".gbc") == 0) {
  romDir = SD_ROM_GBC_DIR;
  }
