 ...
 *
 * Proven on this ILI9341: lcdReadReg puts command+data in ONE transaction (CS
 * low throughout) and reads back exactly what was written. Splitting commands
 * and their data across transactions causes the panel to discard the operation.
 *
 * Nested so lcdSetWindow can be called inside a blit without releasing CS.
 */
...
static void lcdInitILI9341(void) {
  /*
   * ILI9341 initialization from the user's known-working Retro-Go
   * configuration for this exact display.
   */
  ...
  /* Known-good orientation from Retro-Go */
  lcdCmd8(0x36);
  lcdDat8(0x08);
...
/* This handheld has no battery-voltage ADC. */
